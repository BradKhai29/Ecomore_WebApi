...
{
    internal class SystemAccountTokenService :
        ISystemAccountTokenService
    {
        // Backing fields.
        ...
        public SystemAccountTokenService(
            ...
            Guid tokenId)
        {
            var claims = new List<Claim>
            {
                ...
                new(type: SystemAccountCustomClaimTypes.SystemAccountId, systemAccount.Id.ToString()),
            };
        ...
        public async Task<bool> SaveSystemAccountTokenAsync(
            ...
            CancellationToken cancellationToken)
        {
            ...
            await executionStrategy.ExecuteAsync(operation: async () =>
            {
                ...
                try
                {
                    await _unitOfWork.SystemAccountTokenRepository.AddAsync(
                        newEntity: systemAccountToken,
                        cancellationToken: cancellationToken);
        ...
        public async Task<AppResult<SystemAccountTokenEntity>> ValidateAccessTokenAsync(
            string accessToken,
            CancellationToken cancellationToken)
        {
            ...
            {
                return AppResult<SystemAccountTokenEntity>.Failed(AppResult.DefaultMessage.InvalidAccessToken);
            }

            var systemAccountIdClaim = validationResult.ClaimsIdentity.FindFirst(
            ...
            if (Equals(systemAccountIdClaim, null) || Equals(jtiClaim, null) || Equals(expClaim, null))
            {
                return AppResult<SystemAccountTokenEntity>.Failed("SystemAccountId or TokenId or Exp claim has invalid format.");
            }

            var isValidSystemAccountId = Guid.TryParse(systemAccountIdClaim.Value, out Guid systemAccountId);
            ...
            if (!isValidSystemAccountId || !isValidTokenId || !isValidExpClaim)
            {
                return AppResult<SystemAccountTokenEntity>.Failed("SystemAccountId or TokenId or Exp claim has invalid format.");
            }

            // Check if the tokenId that encapsulated in this token is existed or not.
            var isTokenFound = await _unitOfWork.SystemAccountTokenRepository.IsFoundByExpressionAsync(
                findExpresison: token => token.Id == tokenId,
                cancellationToken: cancellationToken);

            if (!isTokenFound)
            {
                return AppResult<SystemAccountTokenEntity>.Failed("TokenId is not found.");
            }

            var expiredDateTime = DateTimeOffset.FromUnixTimeSeconds(expInSeconds).DateTime;
            var systemAccountToken = new SystemAccountTokenEntity
            {
                Id = tokenId,
            ...
            return AppResult<SystemAccountTokenEntity>.Success(systemAccountToken);
        }

        public async Task<AppResult<SystemAccountTokenEntity>> ValidateRefreshTokenAsync(
            string refreshToken,
            CancellationToken cancellationToken)
        {
            var validationResult = await InternalValidateTokenAsync(
            ...
            if (!validationResult.IsValid)
            {
                return AppResult<SystemAccountTokenEntity>.Failed(AppResult.DefaultMessage.InvalidRefreshToken);
            }
            ...
            if (Equals(systemAccountIdClaim, null) || Equals(jtiClaim, null))
            {
                return AppResult<SystemAccountTokenEntity>.Failed("SystemAccountId or TokenId claim has invalid format.");
            }

            var isValidSystemAccountId = Guid.TryParse(systemAccountIdClaim.Value, out Guid systemAccountId);
            var isValidTokenId = Guid.TryParse(jtiClaim.Value, out Guid tokenId);

            if (!isValidSystemAccountId || !isValidTokenId)
            {
                return AppResult<SystemAccountTokenEntity>.Failed("SystemAccountId or TokenId claim has invalid format.");
            }
        ...
        public async Task<AppResult<SystemAccountTokenEntity>> ValidateRegisterConfirmationTokenAsync(
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!validationResult.IsValid)
            {
                return AppResult<SystemAccountTokenEntity>.Failed(AppResult.DefaultMessage.InvalidToken);
            }
        ...
        public async Task<AppResult<SystemAccountTokenEntity>> ValidateResetPasswordTokenAsync(
        ...
        public SystemAccountTokenEntity CreateSystemAccountTokenByPurpose(
            Guid tokenId,
            SystemAccountEntity systemAccount,
            TokenPurpose tokenPurpose)
        {
            ...
            switch (tokenPurpose)
            {
                case TokenPurpose.RefreshToken:
                    systemAccountToken.Name = TokenNames.RefreshToken;
                    systemAccountToken.ExpiredAt = dateTimeUtcNow.AddDays(_refreshTokenOptions.LiveDays);

                    break;

                case TokenPurpose.RegisterConfirmationToken:
                    systemAccountToken.Name = TokenNames.RegisterConfirmationToken;
                    systemAccountToken.ExpiredAt = dateTimeUtcNow.AddMinutes(_registerConfirmationTokenOptions.LiveMinutes);

                    break;

                case TokenPurpose.ResetPasswordToken:
                    systemAccountToken.Name = TokenNames.ResetPasswordToken;
                    systemAccountToken.ExpiredAt = dateTimeUtcNow.AddMinutes(_registerConfirmationTokenOptions.LiveMinutes);

                    break;
            }

            systemAccountToken.Name = $"{systemAccountToken.Name}.{tokenId}";

            return systemAccountToken;
        }

        public async Task<bool> RemoveSystemAccountTokenByIdAsync(
            Guid tokenId,
            CancellationToken cancellationToken)
        {
            var result = false;
            ...
            await executionStrategy.ExecuteAsync(operation: async () =>
            {
                ...
                try
                {
                    await _unitOfWork.SystemAccountTokenRepository.BulkDeleteByTokenIdAsync(
                        tokenId: tokenId,
                        cancellationToken: cancellationToken);
